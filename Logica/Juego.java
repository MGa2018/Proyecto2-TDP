...
public class Juego {
	...
	public boolean comprobarSolucion() {				
     ...
     public boolean estaRepetida(Celda c) {
	    	boolean rta = false;
	    	boolean encontre = false;
	    	for(int i=0;i<this.getCantFilas();i++) {
				for (int j = 0;j<this.getCantFilas();j++) {//con estos for encuentro el i y el j
					if ((c == tablero[i][j] && !encontre) && (tablero[i][j].getValorInicial()==false)) {
	    	           if (((estaEnLaFila(c.getValor(),i) || estaEnLaCol(c.getValor(),j)) || estaEnElPanel(c.getValor(),i,j))) {
	    		          rta = true;
	    		          encontre = true;
		...
		private boolean estaEnLaFila(int k, int i) {
			...
			boolean rta = false;
			for (int j = 0;j<this.getCantFilas();j++)
				if (tablero[i][j].getValor() == k && tablero[i][j].getValor() != 11)
					cant++;
			if (cant>1)
			     rta = true;
			return rta;
		...
		private boolean estaEnElPanel(int k,int i, int j) {
			...
			for (int m = filacuadrante;m<finfila;m++)
				for (int n = columnacuadrante;n<fincol;n++) {
					if (tablero[m][n].getValor() == k && tablero[i][j].getValor() != 11)
						cant++;
				}
					if (cant>1)
				esta = true;
